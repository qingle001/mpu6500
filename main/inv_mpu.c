...
int mpu_read_6500_accel_bias(long *accel_bias) {
	unsigned char data[6];
	if (i2c_read(0x77, 2, &data[0]))
		return -1;
	if (i2c_read(0x7A, 2, &data[2]))
		return -1;
	if (i2c_read(0x7D, 2, &data[4]))
		return -1;
...
int mpu_read_6500_gyro_bias(long *gyro_bias) {
	unsigned char data[6];
	if (i2c_read(0x13, 2, &data[0]))
		return -1;
	if (i2c_read(0x15, 2, &data[2]))
		return -1;
	if (i2c_read(0x17, 2, &data[4]))
		return -1;
...
int mpu_set_accel_bias_6050_reg(const long *accel_bias) {
    ...
    if (i2c_write(0x06, 2, &data[0]))
        return -1;
    if (i2c_write(0x08, 2, &data[2]))
        return -1;
    if (i2c_write(0x0A, 2, &data[4]))
        return -1;
...
int mpu_set_accel_bias_6500_reg(const long *accel_bias) {
    ...
    if (i2c_write(0x77, 2, &data[0]))
        return -1;
    if (i2c_write(0x7A, 2, &data[2]))
        return -1;
    if (i2c_write(0x7D, 2, &data[4]))
        return -1;
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    ...
    if (i2c_write(st.reg->fifo_en, 1, data))
        return -1;
    vTaskDelay(test.wait_ms / portTICK_RATE_MS);
    data[0] = 0;
    if (i2c_write(st.reg->fifo_en, 1, data))
        return -1;
...
static int accel_6500_self_test(long *bias_regular, long *bias_st)
{
    ...
    if (i2c_read(REG_6500_XA_ST_DATA, 3, regs)) {
    	//if(debug)
...
static int gyro_6500_self_test(long *bias_regular, long *bias_st)
{
    ...
    if (i2c_read(REG_6500_XG_ST_DATA, 3, regs)) {
    	// if(debug)
    	// 	log_i("Reading OTP Register Error.\n");
        return 0x07;
    }
...
static int get_st_6500_biases(long *gyro, long *accel, unsigned char hw_test, int debug)
{
    ...
    vTaskDelay(test.wait_ms / portTICK_RATE_MS);//wait 200ms for sensors to stabilize
    ...
    while (s < test.packet_thresh) {
    	vTaskDelay(test.sample_wait_ms/ portTICK_RATE_MS);//wait 10ms to fill FIFO
		if (i2c_read(st.reg->fifo_count_h, 2, data))
			return -1;
		fifo_count = (data[0] << 8) | data[1];
		packet_count = fifo_count / MAX_PACKET_LENGTH;
		...
		if (i2c_read(st.reg->fifo_r_w, read_size, data))
						return -1;
		ind = 0;
		for (ii = 0; ii < packet_count; ii++) {
			short accel_cur[3], gyro_cur[3];
